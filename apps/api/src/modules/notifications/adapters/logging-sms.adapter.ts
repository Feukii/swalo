...
/**
 * Default SMS adapter: journalises the message and returns ok without any
 ...
 */
@Injectable()
export class LoggingSmsAdapter implements NotificationChannelAdapter {
  private readonly logger = new Logger(LoggingSmsAdapter.name);

  send(recipient: string, subject: string, body: string): Promise<ChannelSendResult> {
    this.logger.log(`[SMS:queued] to=${recipient} subject="${subject}" body="${body}"`);
