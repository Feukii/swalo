        ...
        <div className="w-full max-w-2xl">
          <div className="bg-white rounded-3xl shadow-2xl p-8" id="printable-section">
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 text-sm">Conservez précieusement ces codes d'accès</p>
            </div>
            ...
            <div className="space-y-6 mb-8">
              <div className="bg-primary-50 border-2 border-primary-200 rounded-2xl p-6">
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-xl font-bold text-gray-900">{result.shop.name}</p>
                  </div>
                  <div>
                    <label className="text-sm font-semibold text-gray-600 block mb-1">Devise</label>
                    <p className="text-lg text-gray-900">{result.shop.currency}</p>
                  </div>
                </div>
              </div>

              <div className="bg-blue-50 border-2 border-blue-200 rounded-2xl p-6">
                <h2 className="text-lg font-bold text-blue-900 mb-4">🔑 Code d'Accès Boutique</h2>
                <div className="flex items-center justify-between bg-white rounded-xl p-4 border-2 border-blue-300">
                  <div>
                    <label className="text-sm font-semibold text-gray-600 block mb-1">
                      Code Boutique
                    </label>
                    <p className="text-4xl font-bold font-mono tracking-widest text-blue-600">
                      {result.shop.code}
                    </p>
                  </div>
                  <button
                    onClick={() => handleCopy(result.shop.code)}
                    className="p-3 bg-blue-100 hover:bg-blue-200 rounded-xl transition-colors print:hidden"
                    title="Copier"
                  >
                    <svg
              ...
              <div className="bg-amber-50 border-2 border-amber-200 rounded-2xl p-6">
                <h2 className="text-lg font-bold text-amber-900 mb-4">👤 Propriétaire</h2>
                <div className="space-y-3">
                  <div>
                    <label className="text-sm font-semibold text-gray-600 block mb-1">Nom</label>
                    <p className="text-lg text-gray-900">{result.owner.name}</p>
                  </div>
                  {result.owner.phone && (
                    <div>
                      <label className="text-sm font-semibold text-gray-600 block mb-1">
                      ...
                      <p className="text-lg text-gray-900">{result.owner.phone}</p>
                    </div>
                  )}
                  <div className="flex items-center justify-between bg-white rounded-xl p-4 border-2 border-amber-300 mt-4">
                    <div>
                      <label className="text-sm font-semibold text-gray-600 block mb-1">
                        Code PIN Propriétaire
                      </label>
                      <p className="text-4xl font-bold font-mono tracking-widest text-amber-600">
                        {result.owner.pin_code}
                      </p>
                    </div>
                    <button
                      onClick={() => handleCopy(result.owner.pin_code)}
                      className="p-3 bg-amber-100 hover:bg-amber-200 rounded-xl transition-colors print:hidden"
                      title="Copier"
                    >
                      <svg
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-2xl p-6 mb-6">
              <h3 className="font-bold text-gray-900 mb-3">📝 Instructions pour le propriétaire</h3>
